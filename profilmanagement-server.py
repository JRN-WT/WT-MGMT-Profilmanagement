...
class Store:
    def __init__(self, root):
        ...
        for directory in (self.p, self.v, self.d, self.a): directory.mkdir(parents=True, exist_ok=True)
    def read(self, path):
    ...
    def normalize(self, key, items):
        ...
        for item in items if isinstance(items, list) else []:
            ...
            if key == 'projekte': out.append(project(item)); continue
            value=cp(item); value['id']=str(value.get('id') or uuid.uuid4().hex); out.append(value)
        return out
    def content(self, profile): return {key: cp(profile.get(key, {} if key == 'kurzprofil' else [])) for key in FIELDS}
    def hints(self, profile):
        hints=[]; person=profile.get('person', {})
        if not person.get('vorname') or not person.get('nachname'): hints.append('Name unvollständig')
        for key, label in [('kurzprofil','Kurzprofil'),('kompetenzen','Kompetenzen'),('projekte','Projekterfahrung'),('qualifikationen','Qualifikationen'),('sprachen','Sprachen')]:
            if not profile.get(key): hints.append(label+' noch leer')
        return hints
    def get(self, pid):
        path=self.pp(pid)
        ...
        profile['hinweise']=self.hints(profile)
        return profile
    def list(self):
        result=[]
        ...
        return sorted(result, key=lambda entry: entry['name'].casefold())
    def create(self, data, actor):
        ...
        timestamp=now(); profile={'schemaVersion':'2.1','id':pid,'revision':1,'status':'Entwurf','erstelltAm':timestamp,'erstelltVon':actor,'aktualisiertAm':timestamp,'aktualisiertVon':actor,'person':{'vorname':first,'nachname':last,'titel':'','rolle':str(person.get('rolle','')),'standort':'','interneEmail':'','telefon':'','foto':None},'kurzprofil':{'positionierung':'','zusammenfassung':''},'kompetenzen':[],'branchen':[],'projekte':[],'qualifikationen':[],'sprachen':[],'dokumente':[],'historie':[{'zeitpunkt':timestamp,'aktion':'Profil angelegt','von':actor,'details':'Manuell als Entwurf angelegt.'}]}
        self.write(self.pp(pid),profile); return self.get(pid)
    def validate_projects(self, projects):
        for number, entry in enumerate(projects, 1):
            missing=[]
            if not entry['titel'].strip(): missing.append('Titel')
            if not valid_month(entry['startMonat']): missing.append('Startmonat')
            if not entry['laufend'] and not valid_month(entry['endeMonat']): missing.append('Endmonat oder „laufend“')
            if not entry['rolle'].strip(): missing.append('Rolle')
            if not entry['beschreibung'].strip(): missing.append('Kurzbeschreibung')
            if not entry['aufgaben'].strip(): missing.append('Tätigkeiten')
            if missing: raise ValueError(f'Projekt {number}: '+', '.join(missing)+' ergänzen.')
            if not entry['laufend'] and entry['endeMonat'] < entry['startMonat']: raise ValueError(f'Projekt {number}: Der Endmonat liegt vor dem Startmonat.')
    def mark(self, pid, revision, timestamp, actor):
        profile=self.get(pid); current=self.content(profile)
        for path in self.vf(pid).glob('*.json'):
            ...
            for key in LISTS:
                ...
                choices[key]=[{'id':entry['id'],'sichtbar':bool(old[entry['id']]['sichtbar']) if entry['id'] in old else False,'reihenfolge':old[entry['id']].get('reihenfolge', index+1) if entry['id'] in old else index+1} for index,entry in enumerate(current[key])]
            variant['inhalte']=cp(current); variant['auswahl']=choices; self.normvar(variant)
            variant['pruefungErforderlich']=True; variant['status']='Prüfung erforderlich'; variant['basisRevisionAktuell']=revision; variant['aktualisiertAm']=timestamp; variant['aktualisiertVon']=actor
            variant.setdefault('historie',[]).append({'zeitpunkt':timestamp,'aktion':'Basisprofil aktualisiert','von':actor,'details':'Basisprofil geändert; Variante zur Prüfung markiert.'}); self.write(path,variant)
    def update(self, pid, data, actor):
        ...
        changed=before != self.content(profile); timestamp=now(); profile['aktualisiertAm']=timestamp; profile['aktualisiertVon']=actor
        details='Stammdaten gespeichert; Profilinhalte unverändert.'
        if changed:
            profile['revision']=int(profile.get('revision',1))+1; details='Profilinhalte aktualisiert.'
        profile.setdefault('historie',[]).append({'zeitpunkt':timestamp,'aktion':'Basisprofil gespeichert','von':actor,'details':details}); self.write(self.pp(pid),profile)
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--host',default='127.0.0.1'); parser.add_argument('--port',type=int,default=8081); parser.add_argument('--data-dir',type=Path,default=ROOT/'daten'); args=parser.parse_args()
    server=ThreadingHTTPServer((args.host,args.port),Handler); server.store=Store(args.data_dir); print(f'Profilmanagement: http://{args.host}:{args.port}'); server.serve_forever()
