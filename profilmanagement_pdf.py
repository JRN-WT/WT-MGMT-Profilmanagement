...
def months(project):
    start, end = project.get('startMonat', ''), project.get('endeMonat', '')
    def display(value): return f'{value[5:7]}/{value[:4]}' if len(value) == 7 else ''
    return ' · '.join(part for part in [display(start), 'laufend' if project.get('laufend') else display(end), project.get('rolle', '')] if part)
...
def export_pdf(profile, variant, target):
    ...
    styles.add(ParagraphStyle(name='Section', fontName=bold, fontSize=9, leading=12, textColor=BLUE, spaceBefore=11, spaceAfter=5))
    ...
    def section(title, content): return [Paragraph(title.upper(), styles['Section']), content]
    ...
    if contacts: story += [paragraph(' · '.join(contacts), 'Meta'), Spacer(1, 5)]
    ...
    for key, heading in [('kompetenzen', 'Kompetenzen'), ('branchen', 'Branchen'), ('qualifikationen', 'Qualifikationen & Zertifikate'), ('sprachen', 'Sprachen')]:
        ...
        if rows:
            values = [row.get('sprache', '') + (f" · {row.get('niveau')}" if row.get('niveau') else '') if key == 'sprachen' else row.get('bezeichnung', '') for row in rows]
    ...
    if projects:
        ...
        for project in projects:
            ...
            if project.get('aufgaben'): block += [Spacer(1, 3), paragraph('Tätigkeiten: ' + project['aufgaben'])]
            blocks.append(KeepTogether(block + [Spacer(1, 8)]))
        story += section('Projekterfahrung', blocks)
