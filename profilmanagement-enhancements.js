(() => {
  ...
  const originalChoiceSection = window.choiceSection;
  window.choiceSection = function(key) {
    const markup = originalChoiceSection(key);
    const holder = document.createElement('div');
    holder.innerHTML = markup;
    const heading = holder.querySelector('h3');
    if (heading) {
      ...
      hint.className = 'variant-section-hint';
      hint.textContent = 'Sichtbarkeit und Reihenfolge';
      heading.append(titleNode, hint);
    }
